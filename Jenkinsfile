pipeline {
    ...
    stages {
        stage("Continuous Integration / Intégration Continue") {
            steps {
                git branch: "main", url: "https://github.com/Elvi-2202/My-bank-frontend.git"
                sh "npm install"
                sh "npm run build"
            }
        }
        stage("Continuous Delivery / Livraison Continue") {
            steps {
                sh "docker build . -t ${DOCKERHUB_USERNAME}/my-bank-frontend"
                ...
                sh "docker push ${DOCKERHUB_USERNAME}/my-bank-frontend"
            }
        }
    }
}